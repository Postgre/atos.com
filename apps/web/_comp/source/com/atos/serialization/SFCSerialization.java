...
public class SFCSerialization extends BasePodPlugin {
	...
	private static Logger log = Logger.getLogger(SFCSerialization.class
			.getName());
				...
				if (isStarted) {
					...
					try {
						...
						if (sfcList == null || sfcList.size() == 0) {
							MessageHandler.handle(
									"Select atleast one SFC to perform serialize operation .",
									null, MessageType.ERROR, this);
							return;
						}
						if (sfcList.size() > 1) {
						...
						if (!(this.serializedSFCvalue.length() == 15)) {
							MessageHandler.handle(
									"EIN should be 15 digits value .", null,
									MessageType.ERROR, this);
							return;
						}
						FindSfcByNameRequest sfcNameRequest = new FindSfcByNameRequest();
						sfcNameRequest.setSfc(serializedSFCvalue);
						SfcBasicData sfcbasicdata = new SfcBasicData();
						sfcbasicdata = sfcStateService
								.findSfcByName(sfcNameRequest);
						if (sfcbasicdata != null) {
							MessageHandler.handle(
									"Given EIN " +this.serializedSFCvalue+ "is already serialized .", null,
									MessageType.ERROR, this);
							return;
						}
						...
						serializeSfcDetail.setSfc(serializedSFCvalue);
						List<SerializeSfcDetail> newSfcList = new ArrayList<SerializeSfcDetail>();
						newSfcList.add(serializeSfcDetail);
						SerializeSfcRequest serializeSfcRequest = new SerializeSfcRequest(
								sfcRef);
						serializeSfcRequest.setNewSfcList(newSfcList);
						serializeSfcRequest
								.setQuantityToSerialize(new BigDecimal(1));

						Collection<SerializeSfcResponse> serializeResponse = serializeServiceInterface
								.serializeSfc(serializeSfcRequest);
						closePlugin();
						if (serializeResponse.size() > 0) {
							...
							sfcSelection.setInputId(serializedSFCvalue);
							sfcsSerialized.add(sfcSelection);
							getPodSelectionModel().setSfcs(sfcsSerialized);
							SfcChangeEvent sfcChangeEvent = new SfcChangeEvent(
									this);
							this.fireEvent(sfcChangeEvent,
									SfcChangeListenerInterface.class,
									"processSfcChange");
						}
