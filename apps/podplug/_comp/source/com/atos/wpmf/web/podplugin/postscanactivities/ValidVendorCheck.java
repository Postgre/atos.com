...
public class ValidVendorCheck {
		...
		try{
			...
			// get as built summary data for given product
				...
				for(AssemblyDataValidationsEntity assemblyDataValidationsEntity:assemblyDataValidationsEntities){	
					...
					asBuiltComponentsRequest.setOperationRef(assemblyDataValidationsEntity.getCheckOperation());
					asBuiltComponentsRequest.setComponentName(product);
					...
					if(assemblyDataValidationsEntity.getValidateFor().equals(SFC)){
						...
						//iterating already assembled components
						for(AssembledComponent assembledComponent:assembledComponents){
							    //iterating data fields of already assembled components							
									for(AssemblyDataField assemblyDataField:assembledComponent.getAssemblyDataFields()){
	 ...
	 * *******************************DatabaseCode*******************************************/
			...
			public List<AssemblyDataValidationsEntity> findAssemblyValidations(String operation, String material) {
				...
				Connection con = null;
				String query = "SELECT FIRST_OPERATION , MATERIAL , CHECK_OPERATION , CHECK_FIELD ,VALIDATE , VALIDATE_FOR  , SFC FROM ASSEMBLY_DATA_VALIDATIONS where FIRST_OPERATION like '%"+operation+"%' and MATERIAL = ? and VALIDATE = 'YES'";
